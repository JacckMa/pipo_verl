...
To reproduce the results on the SciKnowEval dataset:

**GRPO (Baseline):**
```bash
./scripts/run_experiment.sh experiments/sciknoweval/grpo_baseline
```

**GRPO + PIPO (Ours):**
```bash
./scripts/run_experiment.sh experiments/sciknoweval/grpo_pipo
```
...
| `loss_scale_neg` (λ) | **0.1** | Controls suppression strength for negative updates |
