...
from verl.trainer.ppo.layback_utils import (
    LaybackRewardHistory,
    SDPODistillLaybackCache,
    compute_linear_layback_coefficient,
)
...
class RayPPOTrainer:
    ...
    def __init__(
        ...
        self.layback_config = self.config.algorithm.get("layback", None)
        self.layback_cache = None
        self.layback_reward_history = None
        if self.layback_config is not None and self.layback_config.get("enable", False):
            loss_mode = self.config.actor_rollout_ref.actor.policy_loss.get("loss_mode", "vanilla")
            self_distillation_cfg = self.config.actor_rollout_ref.actor.get("self_distillation", {})
            sdpo_beta = self_distillation_cfg.get("beta", 0.9)
            if loss_mode != "sdpo" or sdpo_beta != 0.0:
                raise ValueError("PIPO for SDPO requires policy_loss.loss_mode=sdpo and self_distillation.beta=0.0.")
            self.layback_cache = SDPODistillLaybackCache()
            self.layback_reward_history = LaybackRewardHistory(
                window_size=self.layback_config.get("history_window_size", 8)
            )
    ...
    def _validate(self, merged: bool = False):
        ...
        invalid_extra_info_keys = []
        for key_info, lst in reward_extra_infos_dict.items():
            if len(lst) != 0 and len(lst) != len(sample_scores):
                print(
                    f"[validate] drop reward extra info with mismatched length: "
                    f"{key_info}: len={len(lst)}, expected={len(sample_scores)}"
                )
                invalid_extra_info_keys.append(key_info)
        for key_info in invalid_extra_info_keys:
            reward_extra_infos_dict.pop(key_info, None)
    ...
    def _val_metrics_update(self, data_sources, sample_uids, reward_extra_infos_dict, sample_turns):
        ...
        core_mean_values = [
            ...
            if metric_name.startswith("val-core/") and "/mean@" in metric_name
        ]
        core_metric_values = core_mean_values or [
            float(metric_val)
            for metric_name, metric_val in metric_dict.items()
            if metric_name.startswith("val-core/")
        ]
        if core_metric_values:
            metric_dict["val-core/average"] = float(np.mean(core_metric_values))
    ...
    def _sdpo_layback_enabled(self) -> bool:
        return (
            self.layback_config is not None
            and self.layback_config.get("enable", False)
            and self.layback_cache is not None
            and self.layback_reward_history is not None
        )

    @staticmethod
    def _batch_reward_mean(reward_tensor: torch.Tensor) -> float:
        if reward_tensor.dim() > 1:
            reward_tensor = reward_tensor.sum(dim=-1)
        return float(reward_tensor.float().mean().detach().cpu().item())

    def _maybe_apply_sdpo_layback(self, current_reward_mean: float) -> dict[str, float]:
        if not self._sdpo_layback_enabled():
            return {}

        metrics = {
            "layback/enabled": 1.0,
            "layback/cache_hit": float(self.layback_cache.has_cache()),
            "layback/applied": 0.0,
            "layback/cache_step_gap": -1.0,
        }
        if self.layback_cache.has_cache() and self.layback_cache.step_count is not None:
            metrics["layback/cache_step_gap"] = float(self.global_steps - self.layback_cache.step_count)

        min_steps = self.layback_config.get("min_steps_before_layback", 2)
        every_n_steps = self.layback_config.get("layback_every_n_steps", 1)
        if (
            not self.layback_cache.has_cache()
            or len(self.layback_reward_history) < min_steps
            or every_n_steps <= 0
            or self.global_steps % every_n_steps != 0
        ):
            return metrics

        coefficient, coefficient_metrics = compute_linear_layback_coefficient(
            current_mean=current_reward_mean,
            historical_mean_rewards=self.layback_reward_history.previous_rewards(),
            loss_scale_neg=self.layback_config.get("loss_scale_neg", 0.0),
            epsilon=self.layback_config.get("eps", 1e-6),
        )
        metrics.update(coefficient_metrics)
        if coefficient <= 0:
            return metrics

        layback_batch = self.layback_cache.build_batch()
        layback_batch.meta_info["layback_loss_scale"] = coefficient
        layback_batch.meta_info["skip_teacher_update"] = True
        layback_batch.meta_info["is_layback"] = True
        layback_batch.meta_info["layback_disable_distillation_is_clip"] = True
        layback_batch.meta_info["global_token_num"] = torch.sum(
            layback_batch.batch["attention_mask"], dim=-1
        ).tolist()
        layback_batch.meta_info["images_seqlens"] = []

        actor_output = self._update_actor(layback_batch)
        ...
        metrics.update(
            {
                ...
                "layback/sdpo_loss": actor_metrics.get("self_distillation/loss", 0.0),
                "layback/loss_scale": coefficient,
                "layback/teacher_update_skipped": actor_metrics.get("actor/teacher_update_skipped", 0.0),
                "layback/is_clip_skipped": actor_metrics.get("self_distillation/is_clip_skipped_for_layback", 0.0),
            }
        )
        return metrics

    def _cache_sdpo_layback_batch(self, batch: DataProto, current_reward_mean: float) -> dict[str, float]:
        if not self._sdpo_layback_enabled():
            return {}

        metrics = {
            "layback/cache_updated": 1.0,
            "layback/history_size": float(len(self.layback_reward_history)),
            "layback/current_reward_mean": float(current_reward_mean),
        }
        self.layback_cache.update(batch, step_count=self.global_steps)
        self.layback_reward_history.update(current_reward_mean)
        return metrics
            ...
            for batch_dict in self.train_dataloader:
                ...
                with marked_timer("step", timing_raw):
                    ...
                    with marked_timer("adv", timing_raw, color="brown"):
                        ...
                        current_reward_mean = self._batch_reward_mean(reward_tensor)
                    ...
                    if self.config.trainer.critic_warmup <= self.global_steps:
                        # update actor
                        if self._sdpo_layback_enabled():
                            with marked_timer("layback_update_actor", timing_raw, color="red"):
                                layback_metrics = self._maybe_apply_sdpo_layback(current_reward_mean)
                            metrics.update(layback_metrics)
                        with marked_timer("update_actor", timing_raw, color="red"):
                            actor_output = self._update_actor(batch)
                        actor_output_metrics = reduce_metrics(actor_output.meta_info["metrics"])
                        metrics.update(actor_output_metrics)
                        metrics.update(self._cache_sdpo_layback_batch(batch, current_reward_mean))
