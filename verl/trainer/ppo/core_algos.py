...
def compute_policy_loss_vanilla(
    ...
    layback_clip_ratio_high = getattr(config, "layback_clip_ratio_high", None)
    if layback_clip_ratio_high is None and hasattr(config, "get"):
        layback_clip_ratio_high = config.get("layback_clip_ratio_high", None)
    if layback_clip_ratio_high is not None:
        cliprange_high = layback_clip_ratio_high
...
def compute_policy_loss_grpo_layback(
    ...
    """GRPO-style layback loss using cached old_log_probs and layback coefficients as advantages."""
    assert config is not None
    assert not isinstance(config, AlgoConfig)
...
def compute_policy_loss_gspo_layback(
    ...
    """GSPO layback loss with precomputed group-scaled layback advantages."""
    assert config is not None
    assert isinstance(config, ActorConfig)
