...
def compute_self_distillation_loss(
    ...
    layback_loss_weights: Optional[torch.Tensor] = None,
) -> tuple[torch.Tensor, dict[str, Any]]:
    ...
    if layback_loss_weights is not None:
        weights = layback_loss_weights.to(device=per_token_loss.device, dtype=per_token_loss.dtype).view(-1, 1)
        per_token_loss = per_token_loss * weights
        metrics["self_distillation/layback_weight_mean"] = weights.mean().detach().item()
        metrics["self_distillation/layback_weight_nonzero_ratio"] = (weights > 0).float().mean().detach().item()
