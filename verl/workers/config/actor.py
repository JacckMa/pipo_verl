...
class PolicyLossConfig(BaseConfig):
    ...
    Args:
        ...
        bapo_rho_0 (float): Target positive-advantage contribution ratio for BAPO loss.
        bapo_a_minus (float): Initial c_low for BAPO loss (adaptive lower bound).
        bapo_b_minus (float): Hard upper bound for c_low in BAPO loss.
        bapo_a_plus (float): Initial c_high for BAPO loss (adaptive upper bound).
        bapo_b_plus (float): Hard upper bound for c_high in BAPO loss.
        bapo_delta_1 (float): Step size for expanding c_high in BAPO loss.
        bapo_delta_2 (float): Step size for expanding c_low in BAPO loss.
    """
    ...
    # BAPO parameters (aligned with official BAPO)
    bapo_rho_0: float = 1.0
    bapo_a_minus: float = 0.6
    bapo_b_minus: float = 0.8
    bapo_a_plus: float = 1.2
    bapo_b_plus: float = 2.0
    bapo_delta_1: float = 0.05
    bapo_delta_2: float = 0.05
