...
class _ConfigOverride:
    def __init__(self, base_config, overrides: dict):
    ...
    def get(self, name, default=None):
        if name in self.overrides:
            return self.overrides[name]
        if hasattr(self.base_config, "get"):
            return self.base_config.get(name, default)
        return getattr(self.base_config, name, default)
...
class DataParallelPPOActor(BasePPOActor):
    ...
    def update_policy(self, data: DataProto):
        ...
        pad_token_id = data.meta_info.get("pad_token_id", 0)
        loss_mode = self.config.policy_loss.get("loss_mode", "vanilla")
        is_layback = data.meta_info.get("is_layback", False)
        layback_loss_scale = float(data.meta_info.get("layback_loss_scale", 1.0))
        skip_teacher_update = data.meta_info.get("skip_teacher_update", False)
        disable_layback_is_clip = data.meta_info.get("layback_disable_distillation_is_clip", False)
        ...
        if (not self_distillation_enabled) or "old_log_probs" in data.batch.keys():
        ...
        if "layback_loss_weights" in data.batch.keys():
            select_keys.append("layback_loss_weights")
        ...
        for _ in range(self.config.ppo_epochs):
            for batch_idx, mini_batch in enumerate(mini_batches):
                ...
                for micro_batch in micro_batches:
                    ...
                    elif old_log_prob is None or advantages is None:
                        raise ValueError("old_log_probs and advantages are required for non-SDPO policy updates.")
                    ...
                    layback_loss_weights = model_inputs.get("layback_loss_weights", None)

                    if self_distillation_enabled:
                        grpo_beta = self_distillation_cfg.get("beta", 0.9)
                        if grpo_beta != 0.0:
                            if old_log_prob is None or advantages is None:
                                raise ValueError("old_log_probs and advantages are required when self_distillation.beta > 0.")
                            policy_loss_fn = get_policy_loss_fn("vanilla")
                            base_pg_loss, base_pg_metrics = policy_loss_fn(
                            ...
                            base_pg_loss = log_prob.new_tensor(0.0)
                            base_pg_metrics = {}
                        ...
                        distillation_cfg_for_loss = self_distillation_cfg
                        is_clip_skipped_for_layback = False
                        if is_layback and disable_layback_is_clip:
                            distillation_cfg_for_loss = _ConfigOverride(self_distillation_cfg, {"is_clip": None})
                            is_clip_skipped_for_layback = self_distillation_cfg.get("is_clip", None) is not None
                        sdpo_loss, pg_metrics = compute_self_distillation_loss(
                            student_log_probs=log_prob,
                            teacher_log_probs=teacher_log_prob,
                            response_mask=response_mask,
                            self_distillation_config=distillation_cfg_for_loss,
                            ...
                            loss_agg_mode=loss_agg_mode,
                            rollout_is_weights=rollout_is_weights,
                            layback_loss_weights=layback_loss_weights,
                        )

                        sdpo_beta = 1.0 - grpo_beta
                        pg_loss = grpo_beta * base_pg_loss + sdpo_beta * sdpo_loss
                        if layback_loss_scale != 1.0:
                            pg_loss = pg_loss * layback_loss_scale
                        pg_metrics["actor/grpo_pg_loss"] = base_pg_loss.detach().item()
                        pg_metrics["self_distillation/loss"] = sdpo_loss.detach().item()
                        pg_metrics["self_distillation/beta"] = grpo_beta
                        pg_metrics["self_distillation/grpo_weight"] = grpo_beta
                        pg_metrics["self_distillation/sdpo_weight"] = sdpo_beta
                        pg_metrics["self_distillation/empty_target_batch"] = self_distillation_mask.sum().item() == 0
                        pg_metrics["actor/layback_loss_scale"] = layback_loss_scale
                        pg_metrics["actor/is_layback"] = float(is_layback)
                        pg_metrics["self_distillation/is_clip_skipped_for_layback"] = float(is_clip_skipped_for_layback)
        ...
        self.actor_optimizer.zero_grad()
        if did_update:
            if skip_teacher_update:
                metrics["actor/teacher_update_skipped"] = 1.0
            else:
                metrics["actor/teacher_update_skipped"] = 0.0
