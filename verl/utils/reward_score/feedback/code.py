...
MAX_RECORD_FIELD_CHARS = 4096
DEFAULT_CODE_REWARD_MAX_WORKERS = 1


def _get_code_reward_max_workers():
    try:
        value = int(os.environ.get("CODE_REWARD_MAX_WORKERS", DEFAULT_CODE_REWARD_MAX_WORKERS))
    except Exception:
        value = DEFAULT_CODE_REWARD_MAX_WORKERS
    return max(1, value)


def _truncate_str(value, max_chars=MAX_RECORD_FIELD_CHARS):
    if value is None:
        return None
    try:
...
class _BoundedStringIO(io.StringIO):
    def __init__(self, max_chars=MAX_CAPTURE_CHARS):
        super().__init__()
    ...
    def write(self, s):
        if not isinstance(s, str):
            s = str(s)
        remaining = self.max_chars - self.tell()
        if remaining <= 0:
...
def _sanitize_record(record):
    ...
    for key in ("input", "expected", "actual", "debug"):
        sanitized[key] = _truncate_str(sanitized.get(key))
    return sanitized
...
def _create_sandbox_namespace(extra_globals=None):
    ...
    _debug_buffer = _BoundedStringIO()
...
def _short_trace(e, limit=3):
    ...
    try:
        ...
        lines = [f"{type(e).__name__}: {_truncate_str(e, 512)}"]
        for f in tail:
            if f.line:
                lines.append(f"  {_truncate_str(f.line, 512)}")
            lines.append(f"Line {f.lineno} in {f.name} ({FILENAME})")
        return "\n".join(lines)
    except BaseException:
        return f"{type(e).__name__}: <traceback unavailable>"
...
def run_tests_for_one_example(test_cases, completion, send_conn, sparse_rewards, test_idx):
    ...
    try:
        ...
        try:
            send_conn.send(_sanitize_record(record))
        except Exception as send_e:
            print(f"[{test_idx}] SEND ERROR: {type(send_e).__name__}: {_truncate_str(send_e, 512)}\n")

        # End INNER block

    except BaseException as outer_e:
        # Error in OUTER block
        print(f"[{test_idx}] OUTER EXCEPTION: {type(outer_e).__name__}: {_truncate_str(outer_e, 512)}\n")
        ...
        try:
            ...
            print(f"[{test_idx}] SEND OUTER ERROR: {type(send_e).__name__}: {_truncate_str(send_e, 512)}\n")
...
def run_tests(test_cases: dict, solution, sparse_rewards, max_test_cases):
    ...
    if "inputs" not in test_cases and "assert_case" in test_cases:
        test_cases = dict(test_cases)
        test_cases["inputs"] = test_cases["assert_case"]
        test_cases["outputs"] = test_cases.get("outputs") or [""] * len(test_cases["inputs"])
    ...
    max_workers = min(_get_code_reward_max_workers(), num_test_cases)

    records_by_idx = {}
    active = []
    next_test_idx = 0

    def _start_test(test_idx):
    ...
    def _fallback_result(test_idx, actual, debug=""):
        return {
            "test_idx": test_idx,
    ...
    while next_test_idx < num_test_cases or active:
        while next_test_idx < num_test_cases and len(active) < max_workers:
            _start_test(next_test_idx)
            next_test_idx += 1

        made_progress = False
        now = time.time()
        for data in list(active):
            ...
            if parent_conn.poll(0):
                try:
                    ...
                    result = _fallback_result(test_idx, f"Process Error: {_short_trace(recv_e)}")
            elif not p.is_alive():
                if parent_conn.poll(0.1):
                    ...
                    result = _fallback_result(test_idx, "Process Error: worker exited without a result")
            elif elapsed >= timeout_this_test:
                result = _fallback_result(test_idx, TIMEOUT)

            if result is None:
                continue

            records_by_idx[test_idx] = _sanitize_record(result)
            p.join(timeout=0)
            if p.is_alive():
                p.kill()
                p.join()
    ...
    records = [records_by_idx[i] for i in range(num_test_cases)]
